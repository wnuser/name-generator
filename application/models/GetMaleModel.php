...
class GetMaleModel extends CI_Model
{
	public function index($post)
	{
        ...
        if($beginwith) :
                $sql      = "SELECT name FROM `malenames` WHERE $wString  name LIKE '$beginwith%' limit $limit ";
       ...
       if(count($result))
       {
        ...
        {
                if($beginwith):
                        $sql  = "SELECT name FROM `malenames` WHERE name LIKE '$beginwith%' limit $limit ";
    ...
    /**
        ...
        public function addFirstLetter($data, $beginwith, $surname)
        {
            ...
            return self::formatData($data, $surname);
